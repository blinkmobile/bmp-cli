...
function pullAnswerSpace (options) {
  let interactions;
  return scope.getUID()
    .then((uid) => api.getResource({ uid, type: 'answerspaces' }))
    .then((result) => {
...
function pullInteraction (options) {
  return resource.readCachedAnswerSpace({ cwd: options.cwd })
    .then((data) => api.getResource({
      id: '' + options.id, // ensure id is a string
      type: 'interactions',
      uid: data.name
    }))
