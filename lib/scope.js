...
const ERROR_SCOPE_INVALID = require('./values').ERROR_SCOPE_INVALID;
const ERROR_SCOPE_NOT_SET = require('./values').ERROR_SCOPE_NOT_SET;
...
function findConfig () {
  ...
  return findUp(CONFIG_FILE, { cwd })
    .then((filePath) => {
      if (!filePath) {
        return Promise.reject(new Error(ERROR_SCOPE_NOT_SET));
      }
      return filePath;
    });
}
...
function read () {
  if (process.env.BMP_SCOPE) {
    ...
    if (!urlUtils.isScopeURL(process.env.BMP_SCOPE)) {
      const error = new TypeError(ERROR_SCOPE_INVALID);
      error.data = process.env.BMP_SCOPE;
