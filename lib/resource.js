...
const isFileInReferences = require('@blinkmobile/json-as-files').isFileInReferences;
...
function isSomeFileMatch (refs, dataPath, filePaths) {
  return filePaths.some((filePath) => isFileInReferences(refs, dataPath, filePath));
}
...
/* mutates tree, removes resources without any matching file references */
function matchTreePaths (tree, filePaths) {
  if (!isSomeFileMatch(tree.answerSpace.refs, '', filePaths)) {
    delete tree.answerSpace;
  }
  Object.keys(tree.interactions)
    .forEach((name) => {
      if (!isSomeFileMatch(tree.interactions[name].refs, '', filePaths)) {
        delete tree.interactions[name];
      }
    });
  return tree;
}
