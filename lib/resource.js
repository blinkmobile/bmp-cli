...
const jsonFiles = require('@blinkmobile/json-as-files');
...
function writeAnswerSpace (options) {
  return writeJsonFiles({
    data: options.data,
    defaults: defaultAnswerSpace('answerSpace'),
    filePath: path.join(options.cwd, 'answerSpace.json'),
    fixFn: fixAnswerSpace
  });
}
...
function writeInteraction (options) {
  ...
  return writeJsonFiles({
    data: options.data,
    defaults: defaultInteraction(name),
    filePath: path.join(options.cwd, 'interactions', name, `${name}.json`),
    fixFn: fixInteraction
  });
}

// options = { data, defaults, filePath, fixFn }
function writeJsonFiles (options) {
  const filePath = options.filePath;
  return mergeExistingPlaceholders(filePath, options.defaults)
    .then((template) => {
      const data = Object.assign({}, options.data);
      options.fixFn(data);
      // plan write operations, honoring the $file placeholders
      return jsonFiles.planWriteData({ data, filePath, template });
    })
    .then((plan) => {
      // persist the real data, honoring the $file placeholders
      return jsonFiles.writePlan({ plan: content.tailorPlan(plan) });
    });
}
