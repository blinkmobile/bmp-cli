...
const handleSitemap400 = require('./error.js').handleSitemap400;
...
function assertTypeAnswerspaces (options) {
  if (options.type !== 'answerspaces') {
    throw new TypeError('"type" must be "answerspaces"');
  }
}
...
// just like putResource, but for a limited set of answerSpace data
const wipeSiteMap = decorateWithOptionsAssertions((options) => {
  ...
  return putResource(options)
    .catch(handleSitemap400);
}, [
  assertTypeAnswerspaces
]);
